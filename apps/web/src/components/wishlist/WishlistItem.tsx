...
function pricePresentation(item: WishlistItemDTO, currency: string) {
  if (item.priceMinor != null) return { value: formatMoney(item.priceMinor, currency), listedOnly: false };
  if (item.listedPriceMinor != null && item.listedCurrency) {
    return { value: formatMoney(item.listedPriceMinor, item.listedCurrency), listedOnly: item.listedCurrency !== currency };
  }
  return { value: '—', listedOnly: false };
}
...
export function WishlistListItem(props: { item: WishlistItemDTO; currency: string; fit: WishlistBudgetFit; analyzing: boolean; onEdit: () => void; onAnalyze: () => void; onPurchase: () => void }) {
  ...
  const price = pricePresentation(item, currency);
  const fitMeta = price.listedOnly ? { ...FIT.needs_data, label: `Convert to ${currency}` } : FIT[fit];
  return <motion.article layout initial={{ opacity: 0, y: 5 }} animate={{ opacity: 1, y: 0 }} className="group grid grid-cols-[48px_minmax(0,1fr)_auto] items-center gap-3 border-b border-slate-100 px-3 py-2.5 last:border-0 hover:bg-slate-50/70 dark:border-neutral-800 dark:hover:bg-white/[0.025] sm:grid-cols-[48px_minmax(0,1fr)_140px_120px_auto]">
    ...
    <div className="hidden text-right sm:block" title={price.listedOnly ? `Listed store price. Enter the converted ${currency} amount to include it in budgets.` : undefined}><div className="text-sm font-semibold tabular-nums text-slate-900 dark:text-neutral-100">{price.value}</div><span className={`rounded-full px-1.5 py-0.5 text-[9px] font-medium ${fitMeta.cls}`}>{fitMeta.label}</span></div>
...
export function WishlistCard(props: { item: WishlistItemDTO; currency: string; fit: WishlistBudgetFit; analyzing: boolean; onEdit: () => void; onAnalyze: () => void; onPurchase: () => void }) {
  ...
  return <motion.article layout initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} whileHover={{ y: -2 }} className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm transition-shadow hover:shadow-md dark:border-neutral-800 dark:bg-neutral-900">
    ...
    <div className="p-4"><div className="mb-1 flex items-start justify-between gap-3"><div className="min-w-0"><h3 className="truncate text-sm font-semibold text-slate-900 dark:text-neutral-100">{item.title}</h3><p className="truncate text-[11px] text-slate-400">{item.retailer ?? item.category}</p></div><div className="shrink-0 text-sm font-bold tabular-nums text-slate-900 dark:text-neutral-100" title={price.listedOnly ? `Listed store price. Enter the converted ${currency} amount to include it in budgets.` : undefined}>{price.value}</div></div><div className="mt-3 flex items-center justify-between"><span className={`rounded-full px-2 py-1 text-[10px] font-medium ${fitMeta.cls}`}>{fitMeta.label}</span><Actions item={item} analyzing={analyzing} onEdit={onEdit} onAnalyze={onAnalyze} onPurchase={onPurchase} /></div></div>
  </motion.article>;
}
